...
- **图片降级功能已停用（默认关闭，待删除）**：`dsh.image.fallback` 默认改为 `false`；桥接端改为
  惰性装载——仅在开启时才挂载图片捕获与 fetch 拦截，默认关闭时零装载、零行为变化。
  原因：新版 DSH（RC8 起）已原生支持图片，不再需要插件侧的降级转发；相关代码保留以备后续整块移除。
- 诊断日志保留于桥接握手/降级路径（仅开启时有意义）。
