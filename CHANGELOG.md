...
- **面板网页缩放** `dsh.panel.zoomLevel`（issue #8）：可把内嵌的 DSH 页面整体放大/缩小
  （档位 50% / 60% / 70% / 80% / 90% / 100%，默认 100%；只提供缩小方向——放大方向在数学上必然导致内容右/下留白，而本 issue 的诉求是"字太大"）。实现用 CSS `zoom`
  作用于 iframe，指针命中测试原生生效（不用 `transform: scale`——那会让点击坐标偏移），
  并按 `1/zoom` 放大 iframe 逻辑尺寸，缩放后物理尺寸恰好等于面板可用区域，因此完美铺满、无横向滚动条，且点击命中位置准确（真机几何实测：0.5–1.0 各档位、有/无工具条共 8 种组合，容器内三处取点命中均为 iframe）。
