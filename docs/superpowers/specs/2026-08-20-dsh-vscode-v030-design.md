...
- 模型不支持视觉：prompt 返回 attachment-error（reason=MODEL_DOES_NOT_SUPPORT_IMAGES）。
  桥接客户端插件拦截该次发送并自动降级：
  1. 将图片字节经桥接交给扩展宿主，落盘到 DSH 会话 cwd（默认=工作区根，文件名去碰撞，如 <ts>_<idx>.png）；
...
- 新增下行/上行消息：把 DSH 发送被拒事件告诉桥接（桥接在 DSH 页面内捕获 prompt 的 attachment-error 即可，见 3.5 探测点）。
- 握手、非桥接（普通浏览器）场景：保持原生行为不变。

### 3.5 技术探测点（实现期再确认）
- 桥接如何“看到”用户消息里的图片内容：优先在“附件加入/粘贴/选择”时捕获文件字节（document 级监听 change/drop/paste），与 DSH 自身附件流程解耦；
- 或发送被拒后经 DSH attachment RPC（按 sessionId+attachmentId）取回图片 base64 再落盘——实现时二选一（以更稳为准）。
...
- 图片捕获方式（3.5）实现期二选一，需实测。
- 远端隧道在“无工作区窗口”场景下 asExternalUri 的行为需实测。
- editor/title 图标在特定主题下可读性（用 VS Code 主题色/CDN 图标兜底）。
