...
- **📁 工作区自动选中**：打开面板时自动把 DSH 切换到与当前 VS Code 文件夹同路径的工作区，并打开它的会话——不用再在 DSH 侧边栏里手动找目录。
- **🙈 侧边栏只显示当前工作区**：隐藏其他工作区分组，侧边栏只剩当前项目及其对话，界面更聚焦。
- **修复：Windows 下桥接被自动禁用后无法恢复**：某些情况下 DSH 会在 profile 配置里留下一条「禁用 dsh-vscode-bridge」的残留记录，导致桥接装好了却不生效（表现为：外链点不开、文件路径点不开、工作区同步不工作）。现在安装器会在每次激活时自动清理这条残留。
- **修复：Windows 上 `dsh` 版本信息识别失败**：`findInPathPosix` / `resolveDshPackageJsonPath` 用平台相关的方式拼接 POSIX 路径，在 Windows 上会得到 `\usr\local\bin\dsh` 这种错误结果。

### 工作区自动选中：它做了什么、怎么用

**背景**：DSH 侧边栏会列出你所有用过的工作区（每个目录一组）。当你在 VS Code 里打开某个文件夹时，插件以前只把这个文件夹作为 `dsh web` 的工作目录，**并不会**让 DSH 的侧边栏跟过去——所以你每次都要在 DSH 侧边栏里手动点一下对应的目录。

**现在**：打开面板时，插件会自动：

1. 幂等注册当前 VS Code 文件夹为 DSH 工作区（已存在则复用，不会重复创建）；
2. 调用 DSH 前端的「打开工作区」能力（与你在侧边栏手动点击是同一个函数），切换到该工作区并打开它的会话——若当前会话已属于目标工作区，则**不会**打断你正在进行的对话；
3. 隐藏侧边栏里其他工作区分组，只留当前这一个。

**截图**：左侧是你的 VS Code 文件夹，右侧 DSH 面板自动选中同名工作区：

| 打开文件夹 → 自动选中 | 侧边栏只剩当前工作区 |
|---|---|
...
**怎么用**：装好插件、打开一个文件夹、点 DSH 图标——就这些，**开箱即用，无需任何配置**。

**如果想关掉**：

| 设置项 | 默认 | 说明 |
|---|---|---|
| `dsh.workspace.autoSelect` | `true` | 关掉后不再自动切换工作区（侧边栏仍会显示全部分组） |
| `dsh.workspace.soloMode` | `true` | 关掉后侧边栏显示全部分组，但保留自动选中 |

**手动操作仍然可用**：自动选中只是「帮你点一下」，DSH 侧边栏的所有原有交互（点其他工作区、新建会话、搜索）都不受影响——切换过去之后，DSH 的当前会话就归你控制，插件不会再抢。

**安全说明**：

- 插件只把**文件夹路径**发给 DSH，不读取、不上传任何文件内容；
- 「只显示当前工作区」是纯粹的**界面隐层**（`display:none`），不删除任何数据、不影响任何会话——关掉开关或清空 VS Code 文件夹即完全还原；
- 若工作区匹配失败（例如你在 DSH 里改过工作区名称），插件会**放弃隐藏、保持原样显示**，绝不会把侧边栏变成空白。
